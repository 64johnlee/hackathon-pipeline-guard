...
**Full description:**
SplunkGuard turns natural-language questions into Splunk investigations. No SPL required — Gemini 2.5 Flash picks the index, writes the search, and returns a structured `SplunkInvestigationReport` with a root cause and paste-ready SPL recommendations. This 2-minute walkthrough runs the **MCP path** (Splunkbase App #7931) end-to-end against a real Splunk Enterprise instance loaded with real GitLab CI/CD telemetry.

What you'll see:
- A standard Splunk-Free Docker instance, set up from scratch in under 5 minutes
- 30 pipelines + 264 jobs (294 events) ingested from the public `gitlab-org/cli` project via HEC — in 37 seconds
- One plain-English question: *"What CI pipelines failed and why?"*
- Gemini autonomously calling the Splunk MCP Server's tools over the Streamable HTTP transport (2 tool calls)
- A structured report that names the specific failing jobs (`code_navigation_golang`, `tests:unit`), infers the incident is ongoing, and returns 3 recommended actions — each with a paste-ready SPL query and confidence level
- Benchmark: `--direct` REST path = 8.48s · MCP path = 37.8s

Built for the Splunk Agentic Ops Hackathon — Observability track + Best Use of Splunk MCP Server.
Code (MIT): https://github.com/64johnlee/hackathon-pipeline-guard · Docs: SPLUNK.md

**Chapters** *(approximate — verify against the final cut; total ≈ 2:14):*
- `0:00` Title — SplunkGuard, Gemini 2.5 Flash + Splunk
- `0:10` The problem: the answer's in Splunk, but you don't know the SPL
- `0:22` Architecture: question → SplunkGuardAgent → Splunk
- `0:36` Setup from scratch (`docker run` + `pip install`)
- `0:50` Step 1 — ingest `gitlab-org/cli` via HEC (30 pipelines / 264 jobs / 294 events, 37s)
- `1:08` Step 2 — ask in plain English (MCP mode)
- `1:15` MCP investigation: Gemini calls `splunk_run_query` twice
- `1:35` Structured `SplunkInvestigationReport` (specific jobs, `is_ongoing: true`)
- `1:58` Recommended action #3 — proactive-monitoring SPL, confidence high
- `2:12` Benchmark table + links

**Tags:** `splunk` · `gemini` · `mcp` · `observability` · `ai-agent` · `devops` · `spl` · `gitlab-ci`
