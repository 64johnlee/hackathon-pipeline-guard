...
SCRIPT: list[Segment] = [
    ...
    Segment(
        lines=[
            _l(""),
            _dim("  # Gemini 2.5 Flash decides which MCP tools to call:"),
            _l(""),
    ...
    Segment(
        lines=[
            ...
            _dim("  │   Gemini 2.5 Flash · tool-call loop ≤15 iter │"),
            _dim("  └──────────────────┬───────────────────────────┘"),
            _dim("                     │  MCP function calls (stdio)"),
            _dim("                     ▼"),
            _dim("  ┌──────────────────────────────────────────────┐"),
            _cyan("  │   pipelineguard.mcp_server (bundled)        │"),
            _dim("  │   list_pipelines · get_pipeline_jobs         │"),
            _dim("  │   get_job_log · create_merge_request_note    │"),
            _dim("  └──────────────────┬───────────────────────────┘"),
            _dim("                     ▼"),
            _dim("              GitLab REST API"),
            _l(""),
            _dim("  Fallback: --direct flag → python-gitlab (single Gemini call)"),
        ],
        hold_frames=FPS * 15,
        typing_speed=1,
    ),
