...
def _green(text: str, bold=False) -> Line:
    return Line(text, GREEN, bold)
...
def _orange(text: str) -> Line:
    return Line(text, ORANGE)
...
SCRIPT: list[Segment] = [
    ...
    Segment(
        lines=[
            ...
            _dim(
                '  → get_pipeline_jobs({"project_id": "myorg/api-service", "pipeline_id": 84321})'
            ),
    ...
    Segment(
        lines=[
            ...
            _dim("  ╭──────────────────────────────── Diagnosis ──────────────────────────────╮"),
            _red(
                "  │  Root cause: REDIS_URL not declared in CI — Redis client falls back    │",
                bold=True,
            ),
            _red(
                "  │             to localhost:6379 where no service runs in CI.             │",
                bold=True,
            ),
